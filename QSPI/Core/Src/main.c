...
#if(!CUSTOM_DEBUG)
	DATASET sensor_data = { .flow_total_ml = 1, .run_time_hours = 2,
			.water_meter_version = 1 };
#else
	my_info myInfo;
	strcpy(myInfo.name, "Dishant\r\n");
	myInfo.age = 20;
	myInfo.gender = 'M';
#endif
...
//  Flash_Erase(QSPI_ERASE_LEN_64KB,BLOCK7_ADDR);
...
#else
	/* Write data to Flash Circular Buffer */
	fcb_status writeStatus = WriteDatasetToFlash(&fcb, &myInfo,
			sizeof(my_info));
  ...
  /* Read data form the flash using circular buffer */
	...
	HAL_Delay(3000);
#else
	/* Read data from Flash Circular Buffer */
//	my_info readInfo;

	memset(&readInfo, 0x00, sizeof(my_info));

	fcb_status readStatus = ReadDatasetFromFlash(&fcb, &readInfo,
			sizeof(my_info));
