...
Wiring_Serial_t Serial = { Serialbegin, Serialclose, SerialWrite,
		SerialAvailable, SerialRead, SerialFlush, SerialprintMode,
		SerialprintString, SerialprintIntegerInBase, Serialputhex,
		SerialprintInteger, SerialprintHexlong, SerialprintOctal,
		SerialprintBinary };
...
void Serialbegin(long baud, uint8_t portNum) {
	...
	case 2:
		...
		delay(500);
	    HAL_UART_Receive_IT(&hlpuart1, (uint8_t *)rx2_temp, 1);
...
case 1:
	...
	break;
case 2:
	HAL_UART_Transmit(&hlpuart1, &c, 1, SERIAL_DEFAULT_TIMEOUT);
	break;
default:
	break;
}
...
switch (portNum) {
case 1:
	return (RX_BUFFER_SIZE_1 + rx_buffer_head1 - rx_buffer_tail1)
			% RX_BUFFER_SIZE_1;
	break;
case 2:
	return (RX_BUFFER_SIZE_2 + rx_buffer_head2 - rx_buffer_tail2)
			% RX_BUFFER_SIZE_2;
	break;
default:
	break;
...
void SerialprintIntegerInBase(unsigned long n, unsigned long base,
	uint8_t portNum) {
...
for (; i > 0; i--)
	SerialprintByte(buf[i - 1] < 10 ? '0' + buf[i - 1] : 'A' + buf[i - 1] - 10,
			portNum);
}
